...
export function RoomLobby({ roomId }: RoomLobbyProps) {
    ...
    if (!hasName) {
        return (
            <div className="min-h-[100dvh] flex items-center justify-center bg-slate-50 p-8">
                <motion.div
        ...
        <div className="fixed inset-0 overflow-y-auto bg-slate-50 scroll-smooth">
            <div className="min-h-full w-full flex flex-col items-center justify-center p-8 relative">
                ...
                <motion.div
                    initial={{ scale: 0.9, opacity: 0 }}
                    animate={{ scale: 1, opacity: 1 }}
                    className="bg-white/80 backdrop-blur-xl border border-white p-6 sm:p-10 rounded-3xl shadow-xl w-full max-w-md mx-auto text-center relative z-10 my-auto"
                >
                    ...
                    <div className="space-y-3 mb-8">
                        {gameState.players.map(p => (
                            ...
                            >
                                ...
                                <span className="font-bold text-slate-700 tracking-wide flex items-center gap-2 truncate max-w-[150px]">
                                    {p.name}
                                </span>
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        {gameState.phase === 'LOBBY' && (
                            <div className="flex flex-wrap gap-2 bg-slate-100 p-2 rounded-xl">
                                <input
                                    className="flex-1 min-w-[120px] bg-white rounded-lg px-3 py-2 text-sm font-bold text-slate-700 outline-none border border-slate-200 focus:border-indigo-400 transition-colors"
                                ...
                                <button
                                    ...
                                    className="bg-indigo-500 text-white px-3 py-2 rounded-lg font-bold text-xs hover:bg-indigo-600 transition-colors shadow-sm whitespace-nowrap"
                                >
                                    + ADD CPU
                                </button>
                            </div>
